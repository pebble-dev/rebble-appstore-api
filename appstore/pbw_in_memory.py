...
class PBW(object):
    ...
    def __init__(self, pbw_object, platform):
        self.platform = platform

        file_like_pbw = io.BytesIO(pbw_object)

        self.zip = zipfile.ZipFile(file_like_pbw)
        self.manifest = None
        self.header = None
...
def release_from_pbw(app, fileObj, release_notes = None, published_date = datetime.datetime.utcnow(), version = '', compatibility = []):
    pbw = PBW(fileObj, 'aplite')
    js_md5 = None
