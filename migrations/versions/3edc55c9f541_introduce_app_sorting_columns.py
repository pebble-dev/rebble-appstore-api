...
def upgrade():
    ...
    op.add_column('apps', sa.Column('decaying_hearts', sa.Float(), nullable=True))
    op.add_column('apps', sa.Column('random_weekly', sa.Integer(), nullable=True))
    op.create_index(op.f('ix_apps_decaying_hearts'), 'apps', ['decaying_hearts'], unique=False)
...
def downgrade():
    ...
    op.drop_index(op.f('ix_apps_decaying_hearts'), table_name='apps')
    op.drop_column('apps', 'random_weekly')
    op.drop_column('apps', 'decaying_hearts')
    # ### end Alembic commands ###
